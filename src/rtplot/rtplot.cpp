...
void RTPlot::create(bool start_stdin_parser) {
	...
	impl_ = unique_ptr<rtplot_members>(new rtplot_members());
...
void RTPlot::setMaxPoints(int count) {
	impl_->plot_->setMaxPoints(count);
}
...
void RTPlot::autoRefresh(bool enable, uint period_ms) {
	using cycle = chrono::duration<int, chrono::milliseconds::period>;
		...
		if(create_thread) {
			impl_->auto_refresh_thread_ = thread(
				...
				while(impl_->auto_refresh_period_) {
				    ...
				    auto end_time = start_time + cycle(period_ms);
